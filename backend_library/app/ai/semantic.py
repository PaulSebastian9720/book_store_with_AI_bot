...
INTENT_RULES: list[tuple[str, list[str]]] = [
    ...
    ("process_payment", [
        ...
        r"(?:realizar|procesar|autorizar|completar)\s+(?:el\s+)?pago",
    ...
    ("confirm_payment", [
        r"^s[ií]\b",
        r"^(?:s[ií],?\s+)?confirm[oa]",
        r"^(?:s[ií],?\s+)?(?:dale|va|claro|listo|ok|okay|okey|seguro|adelante|procede)",
        r"confirm[oa]r?\s+(?:el\s+)?pago",
        r"(?:s[ií],?\s+)?(?:pagar|paga|págalo)",
        r"^dale\s+(?:paga|con\s+el\s+pago)",
        r"^(?:procede|adelante)\s+con\s+el\s+pago",
    ]),
    ("cancel_order", [
    ...
    ("view_cart", [
        r"(?:ver|mostrar|muestra|dame|enseña)\s+(?:mi\s+)?carrit[oa]",
        r"(?:qué|que)\s+(?:hay|tengo)\s+(?:en\s+)?(?:el\s+|mi\s+)?carrit[oa]",
        r"(?:mi\s+)?carrit[oa]$",
        r"contenido\s+(?:de\s+)?(?:mi\s+)?carrit[oa]",
        r"(?:revisar|consultar)\s+(?:mi\s+)?carrit[oa]",
    ]),
    ("get_order_status", [
